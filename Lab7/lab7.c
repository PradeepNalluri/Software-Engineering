...
//Proxy std2
float std2(float* loce,float* da,float length,float b_0,float b_1,float toregrate){
  float predicted=0;
  for(int i=0;i<length;i++){
...
int main(){
  ...
  printf("Initial Value of a is %f and Initial Value of b is %f\n",initial_aval,initial_bval);
}
printf("Beta one: %f\n",betaone);
printf("Beta zero: %f\n",betazero);
float range=0;
float expected=0;
for(int i=0;i<3;i++){
  printf("For the %dth toregrate value %f :\n",i+1,raw_estimates[i]);
  float std=std2(loce,da,num_samples,betazero,betaone,raw_estimates[i]);
  for(int j=0;j<2;j++){
    range=t_value[j]*std;
    printf("range is %f\n",range);
